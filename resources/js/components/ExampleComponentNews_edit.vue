...
export default {
    ...
    data: function () {
        return {
            dropzoneOptions: {
                url: '/news_Dropzone_edit/' + this.id_news,
                maxFiles: 1,
                maxFilesize: 500000,
                timeout: 180000,
                addRemoveLinks: true,
    ...
    methods: {
        ...
        getThumnailImg() {
            this.$emit('get_news_img')
        },
        getThumnailImg1() {
            this.$nextTick(function () {

                if (this.news_file_list) {
                    var file = {
                        size: this.news_file_list.size,
                        name: this.news_file_list.name,
                        path: this.news_file_list.path,
                    };
                    var url = "";
                    this.$refs.myVueDropzone.manuallyAddFile(file, '/uploads/news/' + this.id_news + '/' + this.news_file_list.name);
                }
            });
        },
        removeFile() {
